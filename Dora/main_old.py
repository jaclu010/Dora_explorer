...
class Thread(threading.Thread):
    ...
    def run(self):
        ...
        if (self.typee == 3): main_slam(self.threadID)
t1 = Thread(1, 0)
t2 = Thread(2, 1)
t3 = Thread(3, 2)
slam_thread = Thread(4,3)
            ...
            try:
                if (serUSB0.isOpen()):
                    ...
                    if (serial_state == 1):
                        ...
                        #gyro_state
                        #print("gyro")
                        #print("we be here in the serial state of numbero 3")
                        byte = serUSB0.read(1)
                        byteORD = ord(byte)
...
def main_slam(tid):
    ...
    while running:
        ...
        if (last_gyro_time != last_time_stamp):
            ...
            delta_rot = last_gyro_value * (time.time() - last_gyro_time)
            #print(delta_rot)
            if (math.fabs(delta_rot) >= 0.01):
                robot_rot += math.radians(delta_rot)
                #print(math.radians(delta_rot))
            print(robot_rot)
            
            #print(("test - " , delta_rot))
        """    
        #if deltarot >= max_deltarot:
        #    max_deltarot = deltarot
        #    print (deltarot)
