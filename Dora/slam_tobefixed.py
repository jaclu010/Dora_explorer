...
def mapper(read):
    ...
    for i in range(len(merged_lines)):
        ...
        bad_dx = int(round((bad_dot_1[0] - bad_dot_2[0]) / 40))
        bad_dy = int(round((bad_dot_1[1] - bad_dot_2[1]) / 40))
        diff = bad_dx - bad_dy
        #print(bad_dot_1, bad_dot_2)

        cnt = int(round(nr_cells))
            ...
            if line_scr == -1:
                if diff > 0:
                    for j in range(bad_dx):
                        ...
                        cnt_y = (j - diff) * dir_y
                        if cnt_y < 0: cnt_y = 0
                        add_submap_score(cur_y + cnt_y, cur_x + cnt_x, score, submap)
                else:
