...
def additionalDirections(dir_x, dir_y, s_dir_x, s_dir_y, dist_x, dist_y):
    if abs(dir_x) != abs(s_dir_x) or abs(dir_y) != abs(s_dir_y):
        x = rounds2(dist_x, 0.4)
        y = rounds2(dist_y, 0.4)
        x += dir_x
        y += dir_y
    else:
        x = rounds2(dist_x, 0.5)
...
def mapper(read):
    ...
    if not bad_reading:
        for i in range(len(merged_lines)):
            ...
            start_dot_x = line_start_dot[0] / 40
            start_dot_y = line_start_dot[1] / 40
            ...
            if not first_loop and line_scr_p == -1:
                ...
                new_pos = additionalDirections(dir_x, dir_y, start_dir[0], start_dir[1], new_draw_start_x, new_draw_start_y)
